...
def test_named_input_and_outputs_callable_two_in_two_out():
    ...
    named_outputs = named_input_and_outputs_callable(
        add_subtract, named_inputs=named_inputs, input_names=["in0", "in1"], output_names=["out0", "out1"]
    )
    assert named_outputs == {"out0": 4, "out1": 2}

    named_outputs = named_input_and_outputs_callable(
        add_subtract, named_inputs=named_inputs, input_names={"value0": "in0", "value1": "in1"}, output_names=["out0", "out1"]
    )
    assert named_outputs == {"out0": 4, "out1": 2}

    named_outputs = named_input_and_outputs_callable(
        add_subtract, named_inputs=named_inputs, input_names={"value1": "in1", "value0": "in0"}, output_names=["out0", "out1"]
    )
    assert named_outputs == {"out0": 4, "out1": 2}

    with pytest.raises(TypeError, match="got an unexpected keyword argument 'not_a_name'"):
        named_outputs = named_input_and_outputs_callable(
            add_subtract, named_inputs=named_inputs, input_names={"not_a_name": "in0", "value1": "in1"}, output_names=["out0", "out1"]
        )

    with pytest.raises(AssertionError, match="The number of specified output names"):
        named_outputs = named_input_and_outputs_callable(
            add_subtract, named_inputs=named_inputs, input_names={"value1": "in1", "value0": "in0"}, output_names=["out0"]
        )
...
def test_named_input_and_outputs_callable_all():
    ...
    named_inputs = {"in0": 3, "in1": 1}
    named_outputs = named_input_and_outputs_callable(
        add_sum_and_prod, named_inputs=named_inputs, input_names=["__all__", "in1"], output_names=["out0", "out1", "value"]
    )
    ...
    with pytest.raises(AssertionError, match="The number of specified output names"):
        named_outputs = named_input_and_outputs_callable(
            add_sum_and_prod, named_inputs=named_inputs, input_names=["__all__", "in1"], output_names=["out0"]
        )


def test_parse_argument_loss_output_name_indices():
    available_output_names = ["abc", "def", "ghi"]
    with pytest.raises(AssertionError, match="One or more loss"):
        parse_argument_loss_output_name_indices(loss_output_names=["123"], available_output_names=available_output_names)

    empty = parse_argument_loss_output_name_indices(loss_output_names=[], available_output_names=available_output_names)
    assert empty == []

    empty = parse_argument_loss_output_name_indices(loss_output_names="none", available_output_names=available_output_names)
    assert empty == []

    all = parse_argument_loss_output_name_indices(loss_output_names="all", available_output_names=available_output_names)
    assert all == [0, 1, 2]
